...
- [[#introduction][Introduction]]
  ...
  - [[#adding-my-own-configuration-into-kickstartemacs][Adding My Own Configuration into Kickstart.emacs]]
...
** Adding My Own Configuration into Kickstart.emacs
I sort the content by the first letter.
Add the following packages and configurations:
- Dirvish: Replaces dired and implements most of the functionality of treemacs.
- Project(built-in package): Replaces projectile package, it is enough for me.
...
#+begin_src emacs-lisp
    ...
    ;;   (load-theme
    ;;   'gruvbox-dark-medium t)) ;; We need to add t to trust this package
...
* Development
** CMake
I usually use cmake to develop c/c++ language.
#+begin_src emacs-lisp
...
CMake workflow
#+begin_src emacs-lisp
    (setq my/cmake-default-build-dir "build")
    ;; CMake 项目函数
    (defun my/cmake-detect-build-tool (build-dir)
      (cond
       ((file-exists-p (expand-file-name "build.ninja" build-dir)) "ninja")
       (t "make")))
    ...
    (defun my/cmake-configure (&optional build-dir)
      ...
      (let* ((project-root (my/project-root))
             (build-dir (or build-dir my/cmake-default-build-dir))
             (default-directory project-root))
        (compile (format "cmake -S . -B %s" build-dir))))

    (defun my/cmake-build (&optional target)
      "Build CMake project using ninja or make."
      (interactive "MTarget (empty = all): ")
      (let* ((project-root (my/project-root))
             (build-dir my/cmake-default-build-dir)
             (tool (my/cmake-detect-build-tool (expand-file-name build-dir project-root)))
             (default-directory project-root)
             (target (if (string-empty-p target) "" target)))
        (compile (format "%s -C %s %s" tool build-dir target))))
    ...
    (defun my/cmake-run (&optional exe)
      "Run a target from the build directory."
      (interactive "MExecutable name: ")
      (let* ((project-root (my/project-root))
             (build-dir (expand-file-name my/cmake-default-build-dir project-root))
             (default-directory build-dir))
        (async-shell-command (format "./%s" exe))))
...
#+begin_src emacs-lisp :tangle no
    ;; ============================================
    ;; Dirvish 配置 - 替代 Dired 和 Treemacs
    ;; ============================================
    ...
    (use-package dirvish
      ...
      (setq dirvish-attributes
            '(vc-state subtree-state all-the-icons collapse file-size file-time))
      ...
      ;; 可选：延迟自动打开侧边栏
      ;; (add-hook 'find-file-hook #'my-dirvish-auto-project-sidebar)

      ;; 9. 自定义主题和外观
;;      (set-face-attribute 'dirvish-subtree-guide-face nil :foreground "grey50")
;;      (set-face-attribute 'dirvish-subtree-state-face nil :foreground "deep sky blue")
)


;; 5. 定义侧边栏命令
  (defun my-dirvish-sidebar-toggle ()
    "切换 Dirvish 侧边栏"
    (interactive)
    (if (and (boundp 'dirvish-side) dirvish-side)
        (dirvish-side-find-file)
      (dirvish-side)))

  (defun my-dirvish-project-sidebar ()
    "在当前项目的根目录打开 Dirvish 侧边栏"
    (interactive)
    (when-let ((root (project-root (project-current t))))
      (dirvish-side root)))

  ;; 6. 快速文件操作函数
  (defun my-dirvish-quick-find-file ()
    "快速查找并打开文件"
    (interactive)
  ...
  (defun my-dirvish-search-in-project ()
    "在项目中搜索文件"
    (interactive)
    (when-let ((proj (project-current t)))
      (dirvish-fd (project-root proj))))
      ;; 8. 自动显示项目侧边栏
      (defun my-dirvish-auto-project-sidebar ()
        "在打开项目文件时自动显示项目侧边栏"
        (when (and (featurep 'project)
                   (project-current)
                   (not (window-parameter nil 'window-side)))
          (my-dirvish-project-sidebar)))
    ...
    (use-package project
      ...
      :config
      ;; 项目侧边栏增强
      (defun my-project-dirvish-sidebar ()
        "打开项目文件浏览器侧边栏"
        (interactive)
        (if-let ((proj (project-current)))
            (progn
              (let ((root (project-root proj)))
                (dirvish-side root)
                (message "项目侧边栏: %s" (file-name-nondirectory root))))
          (user-error "不在项目中")))

      ;; 项目文件快速跳转
      (defun my-project-dirvish-find-file ()
        "在项目中查找文件"
        (interactive)
        (if-let ((proj (project-current)))
      ...
      ;; 项目文件搜索
      (defun my-project-dirvish-search ()
        "在项目中搜索文件"
        (interactive)
    ...
    (defun my-dirvish-quick-access ()
      "快速访问常用目录"
      (interactive)
      (let ((dirs `(("Home" . ,(expand-file-name "~"))
        ...
        (let ((choice (completing-read "选择目录: " (mapcar #'car dirs))))
          (when-let ((path (alist-get choice dirs nil nil #'equal)))
            (dirvish path)))))
    ...
    (defun my-dirvish-setup-keybindings ()
      "设置 Dirvish 相关的键绑定"
      ;; 全局快捷键
      (global-set-key (kbd "C-x C-d") 'dirvish-dwim)
      (global-set-key (kbd "C-x d") 'dirvish)
      (global-set-key (kbd "C-x C-f") 'my-dirvish-quick-find-file)

      ;; 项目相关快捷键
      (global-set-key (kbd "C-c p f") 'my-project-dirvish-find-file)
      (global-set-key (kbd "C-c p s") 'my-project-dirvish-search)
      (global-set-key (kbd "C-c p d") 'my-project-dirvish-sidebar)

      ;; Dirvish 侧边栏快捷键
      (global-set-key (kbd "C-c d") 'my-dirvish-sidebar-toggle)
      (global-set-key (kbd "C-c D") 'my-dirvish-project-sidebar)

      ;; 快速操作菜单
      (global-set-key (kbd "C-c f") 'my-dirvish-hydra/body)

      ;; Dirvish buffer 内的快捷键
      (with-eval-after-load 'dirvish
        (define-key dirvish-mode-map (kbd "?") 'my-dirvish-hydra/body)
        (define-key dirvish-mode-map (kbd "C-c C-f") 'dirvish-fd)
        (define-key dirvish-mode-map (kbd "C-c C-r") 'dirvish-roam)
        (define-key dirvish-mode-map (kbd "SPC") 'dirvish-subtree-toggle)
        (define-key dirvish-mode-map (kbd "TAB") 'dirvish-subtree-toggle)))

    ;; 启用键绑定
    (my-dirvish-setup-keybindings)

    ;; ============================================
    ;; 可选：与 Treemacs 类似的外观
    ;; ============================================

    (defun my-dirvish-setup-tree-view ()
      "配置树形视图"
      (setq dirvish-subtree-state-style 'expand-and-collapse
            dirvish-subtree-line-style 'none
            dirvish-header-style 'none)

      ;; 使用 unicode 字符作为树形指示符
      (setq dirvish-subtree-state-format
            '((expand . "▸ ")
              (collapse . "▾ ")
              (unknown . "? "))))

    (my-dirvish-setup-tree-view)

    ;; ============================================
    ;; 文件操作增强
    ;; ============================================

    (defun my-dirvish-smart-actions ()
      "智能文件操作"
      (defun my-dirvish-open-externally ()
        ...
        (when-let ((file (dired-get-file-for-visit)))
          (if (file-directory-p file)
              (dirvish file)
            (start-process "open" nil "xdg-open" file))))
      ...
      (defun my-dirvish-copy-path ()
        ...
        (when-let ((file (dired-get-file-for-visit)))
          ...
          (message "已复制路径: %s" file)))

      ;; 将自定义操作添加到 Dirvish
      (with-eval-after-load 'dirvish
        (define-key dirvish-mode-map (kbd "X") 'my-dirvish-open-externally)
        (define-key dirvish-mode-map (kbd "W") 'my-dirvish-copy-path)))

    (my-dirvish-smart-actions)
    ...
    ;; 在启动时自动加载 Dirvish
    (add-hook 'after-init-hook 'dirvish-global-mode)

    ;; 可选：在特定模式下禁用 Dirvish
    (defun my-disable-dirvish-in-special-buffers ()
      "在特殊缓冲区中禁用 Dirvish"
      (when (derived-mode-p 'magit-mode 'term-mode 'vterm-mode)
        (dirvish-deactivate)))

    (add-hook 'dirvish-mode-hook #'my-disable-dirvish-in-special-buffers)

    (provide 'my-dirvish-config)
#+end_src
...
#+begin_src emacs-lisp :tangle no
    ...
    (defun my/project-recent ()
      "最近项目列表"
      (interactive)
      (let ((projects (project-known-project-roots)))
        (if projects
            (project-switch-project (completing-read "选择项目: " projects))
          (message "没有已知项目"))))

    ;; ====== 项目缓存管理 ======
    (defvar my/project-cache-file
      (expand-file-name "project-cache.el" user-emacs-directory))

    (defun my/save-project-cache ()
      "保存项目缓存"
      (interactive)
      (project-remember-projects-under "~/")
      (project-remember-projects-under "~/projects/"))

    (defun my/clear-project-cache ()
      "清理项目缓存"
      (interactive)
